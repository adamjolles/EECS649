...
def addInts(a, b): # adds two integers and returns the sum
    ...
    aSize = countSize(a)
    bSize = countSize(b)
    size = aSize if aSize < bSize else bSize
    ...
    elif size < 6: # if the sum is less than 10000, it will take 1-7 seconds to send
    ...
    else: # if the sum is greater than 100000000, it will take 17-25 seconds to send
        time.sleep(random.randint(17, 25))

    if random.random() < 0.5: # 50% chance of the sum being wrong by 10%
        sum = sum * 1.1 
    else: sum * 0.9 
    return int(round(sum, 0))

def main(): # main function
    print("Hello, please enter two integers when promptewd. These will be summed together.")
    while True:
        try:
